...
export function ParserDemo() {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      // Transform KPIs to metrics array
      const metrics = Object.entries(result.kpis).map(([label, value]) => ({
      ...
      const { error } = await supabase.from("case_studies").insert({
        ...
        is_visible: true,
        bg_color: result.confidence > 85 ? "#10b981" : "#f59e0b", // Example dynamic color
      })
      ...
      <div className="flex items-center gap-2 mb-6">
        {sourceButtons.map(({ key, label, icon: Icon }) => (
          <button
            ...
            className={`relative flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${source === key
            ...
            <div className="space-y-3">
              {PARSING_STEPS.map((step, i) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`flex-shrink-0 w-7 h-7 rounded-md flex items-center justify-center ${isDone
            ...
            <div className="glass-panel rounded-xl overflow-hidden">
              ...
              <div className="px-6 py-4 border-t border-border/50 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                <button
                  ...
                  className="flex-1 inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-emerald hover:bg-emerald-dim transition-colors text-background text-sm font-medium"
                >
