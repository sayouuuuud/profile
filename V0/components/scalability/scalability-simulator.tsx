            ...
            >
              ...
              <span
                className={`text-[10px] font-medium text-center leading-tight ${
                  node.type === "primary" ? "text-foreground" : "text-muted-foreground"
                }`}
              >
                {node.label}
              </span>
...
export function ScalabilitySimulator({ projectId }: ScalabilitySimulatorProps) {
  const [activeIndex, setActiveIndex] = useState(0)
  const current = scaleLevels[activeIndex]
    ...
    <div className="w-full">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-3">
          ...
          >
            {current.users.toLocaleString()} users
          </motion.span>
        </div>
        ...
        <div className="relative">
          ...
          <div className="h-1.5 bg-muted rounded-full relative">
            <motion.div
              ...
              animate={{ width: `${(activeIndex / (scaleLevels.length - 1)) * 100}%` }}
              transition={{ type: "spring", stiffness: 300, damping: 30 }}
            />
          </div>

          {/* Step buttons */}
          <div className="flex justify-between mt-0 -translate-y-[calc(50%+3px)] relative">
                ...
                >
                  <motion.div
                    className={`w-4 h-4 rounded-full border-2 transition-all duration-300 ${
                      isActive
                        ? "border-emerald bg-emerald scale-125"
                        : isPast
                          ? "border-emerald/50 bg-emerald/30"
                  ...
                  />
                  <span
                    className={`text-xs font-mono mt-3 transition-colors ${
      ...
      <div className="grid lg:grid-cols-5 gap-4">
        ...
        <div className="lg:col-span-3 glass-panel rounded-xl p-6 card-glow">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <ArchDiagram nodes={current.nodes} color={current.color} />
            </motion.div>
          </AnimatePresence>
          ...
          <div className="glass-panel rounded-xl p-6 card-glow">
            ...
            <div className="mb-4">
              <span className="text-3xl font-bold text-foreground font-mono">
                <AnimatedCost value={current.cost.total} />
              </span>
              <span className="text-sm text-muted-foreground">/mo</span>
            </div>
            <CostBreakdown cost={current.cost} />
          </div>
            ...
            <div className="space-y-3">
              {Object.entries(current.performance).map(([key, val]) => (
                ...
                >
                  ...
                  <span className="text-sm font-mono font-medium text-foreground">{val}</span>
                </motion.div>
              ))}
            </div>
          </div>
